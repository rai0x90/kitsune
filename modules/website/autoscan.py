# autoscan module for kitsune's web recon
...
def init_checks(master_switch, outpath):
    global args
    # Argument Parser
    parser = argparse.ArgumentParser(description='kitsune website autoscan module',
    ...
    parser.add_argument("-w", "--wordlist", help="Specify a list of subdomains.", type=str,
                        default="modules/website/wordlists/bitquark_top100k_sublist.txt")
    parser.add_argument("-sw", "--subwordlist", help="Specify a list of child subdomains.", type=str,
                        default="modules/website/wordlists/top1000_sublist.txt")
    parser.add_argument("-e", "--expand", help="Expand the target list recursively.", action="store_true",
                        default=False)
    parser.add_argument("-ln", "--linkedin", help="Extracts emails and employees details from LinkedIn.",
                        action="store_true", default=True)
    ...
    if not (args.target or args.targets):
        ...
        cprint("info", "Example Usage: python3 autoscan.py -t cia.gov", 1)
        exit()
...
def keyloader(keychain, master_switch):
    keyfile = open("modules/website/keylist.kitsune", "rt")  # Read keylist.kitsune File
    ...
    if len(keychain["linkedin_username"]) > 0 and len(keychain["linkedin_password"]) and args.linkedin:
        print("{0}   LinkedIn Module	: [{1}Enabled{2}]{3}".format(Fore.WHITE + Style.BRIGHT, Fore.GREEN, Fore.WHITE, Style.RESET_ALL))
        master_switch.linkedin = True
    else:
        print("{0}   LinkedIn Module	: [{1}Disabled{2}]{3}".format(Fore.WHITE + Style.BRIGHT, Fore.RED, Fore.WHITE, Style.RESET_ALL))
        master_switch.linkedin = False
...
if __name__ == "__main__":
    ...
    now = datetime.now()
    output_path = 'kitsune_web_autoscan' + str(datetime.now().strftime("%d.%m.%y_%H-%M-%S"))
    sw1 = MasterSwitch()
    keychain = dict()
    c1 = Counter()
    ...
    cprint("info", "\n\n[i] kitsune web autoscan is running...\n", 1)  # Success Msg
